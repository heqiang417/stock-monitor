...
"""
每日选股 - BB0.99 策略（v1）
策略条件：大盘弱市70% + 个股RSI<20 + 收盘价≤布林带下轨×0.99 + 放量1.5倍 + T+1开盘买入 + 7天固定持有
...
# BB触底：close <= boll_lower * 0.99（触及或穿透布林下轨）
candidates = db.execute(f"""
    ...
    WHERE trade_date = '{latest}'
      ...
      AND close <= boll_lower * 0.99
    ORDER BY rsi14 ASC
""").fetchall()
print(f"RSI<20 + BB0.99触底候选: {len(candidates)} 只")
...
if picks:
    ...
    best_md = "无候选"
    picks_md = "无满足BB0.99条件的股票，建议观望"

card = {
    "elements": [
        {"tag": "div", "text": {"tag": "lark_md",
            "content": f"**📈 每日选股 {latest} | BB0.99 v10**"}},
        {"tag": "hr"},
        {"tag": "div", "text": {"tag": "lark_md",
