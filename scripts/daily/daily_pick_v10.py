...
每日选股 - Fstop3_pt5 固定止损策略（v10）
策略条件：大盘弱市 + 个股RSI<18 + 收盘价≤布林带下轨 + 放量1.5倍 + T+1开盘买入
...
# BB触底：close <= boll_lower（触及或穿透布林下轨）
candidates = db.execute(f"""
    ...
    WHERE trade_date = '{latest}'
      ...
      AND close <= boll_lower
    ORDER BY rsi14 ASC
""").fetchall()
print(f"RSI<18 + BB触底候选: {len(candidates)} 只")
...
if picks:
    ...
    best_md = "无候选"
    picks_md = "无满足Fstop3_pt5条件的股票，建议观望"

card = {
    "elements": [
        {"tag": "div", "text": {"tag": "lark_md",
            "content": f"**📈 每日选股 {latest} | Fstop3_pt5 v10**"}},
        {"tag": "hr"},
        {"tag": "div", "text": {"tag": "lark_md",
        ...
        {"tag": "hr"},
        {"tag": "div", "text": {"tag": "lark_md",
            "content": f"**Fstop3_pt5** 候选（{len(picks)}只）TOP5："}},
        {"tag": "div", "text": {"tag": "lark_md", "content": best_md}},
        {"tag": "hr"},
        {"tag": "div", "text": {"tag": "lark_md", "content": picks_md}},
        {"tag": "note", "elements": [{"tag": "plain_text",
            "content": f"数据: {latest} | Fstop3_pt5 | 条件单止损3%止盈5% | 不构成投资建议"}]}
    ]
}
