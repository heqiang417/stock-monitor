...
import pandas as pd
import numpy as np
from jqlib.technical_analysis import RSI, BOLL, KDJ
...
def check_buy_signal(context, stock):
    ...
    k, d, j = KDJ(stock, check_date=context.current_dt, N=9, M1=3, M2=3)
    k_val = k.get(stock, np.nan)
    j_val = j.get(stock, np.nan)
