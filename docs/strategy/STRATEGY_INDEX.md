...
> 维护者:OpenClaw | 更新:2026-04-10 | 参考:[EVAL_FRAMEWORK.md](../EVAL_FRAMEWORK.md)
...
| # | 策略名 | 信号条件 | 买入策略 | 卖出策略 | 弱市 | 基本面 | 评估日期 | 状态 |
|---|--------|---------|---------|---------|------|--------|---------|------|
| 1 | **BB1.00** | RSI<20 + BB≤1.00 | 信号日次一交易日开盘买入 | 固定持有7个交易日，于第8个交易日收盘卖出 | ≥70% | TOP300 | 2026-04-07 | 🟢 运行中 |
| 2 | **BB1.02+KDJ** | RSI<20 + BB≤1.02 + KDJ超卖 | 信号日次一交易日开盘买入 | 固定持有7个交易日，于第8个交易日收盘卖出 | ≥70% | TOP500 | 2026-04-07 | 🟢 运行中 |
| 3 | **策略A** | RSI<19 + BB≤1.00 + VOL≥1.1x | 信号日次一交易日开盘买入 | T+2起检查止损3.5% / 止盈4.0%；若5天内未触发，则第6个交易日收盘卖出 | 无 | TOP800 | 2026-04-11 | 🟢 运行中 |
| 4 | **TP4.5** | RSI<20 + BB≤1.00 + VOL≥1.1x | 信号日次一交易日开盘买入 | T+2起检查止损3.5% / 止盈4.5%；若5天内未触发，则第6个交易日收盘卖出 | ≥40% | TOP800 | 2026-04-18 | 🟢 运行中 |

> **注1**: Fstop3_pt5 v10 已于 2026-04-08 降级为历史/对照策略，不再属于正式每日策略池。
> **注2**: 策略A / TP4.5 的回测口径为 **T+1 开盘买，T+2 起按收盘检查 SL/TP，最长持有 H5 后收盘卖出**；生产推送仅发信号，用户需按同口径执行。
...
### 已确认结论
- **Direction7 精修已完成**（2026-04-09）：
  - 229个组合，Qualified 75，Promising 154。
  - 最优骨架确认：`RSI19 + BB1.00 + VOL1.1 + TOP800 + H5 + SL3.5`
  - 两条决赛线：
    1. **主线** `TP4.0 + 无弱市过滤`：train 56.35 / val 75.48 / test 68.46，test Sharpe 3.0476，spread 19.13
    2. **进攻线** `TP4.5 + 弱市0.4`：train 55.94 / val 75.74 / test 67.13，test Sharpe 3.0663，spread 19.8
  - **结论：策略A 已于 2026-04-10 正式上线**（进每日策略池）

### 下一轮方向
- 当前骨架已收敛，两条决赛线均qualified。
- 下一步：验证 TP4.0 vs TP4.5 的实际收益差异，以及是否默认开启弱市过滤。
- 不再大范围发散搜索参数空间。
...
| 日期 | 动作 |
|------|------|
| 2026-04-19 | 核验并统一执行口径：BB1.00 / BB1.02+KDJ 固定持有7天后于第8天收盘卖出；策略A / TP4.5 为 T+2 起检查 SL/TP，H5 到期收盘卖出 |
| 2026-04-18 | TP4.5（RSI20 + BB1.00 + VOL1.1 + weak0.4 + TOP800 + SL3.5/TP4.5/H5）纳入正式策略池 |
